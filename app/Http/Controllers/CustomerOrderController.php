...
class CustomerOrderController extends Controller
{
    ...
    {
        $orders = $customer->orders()->where('is_quotation', '0')->latest()->paginate(10);
        $totalDiscount = $customer->orders()->where('is_quotation', '0')->sum('discount');
        $totalCost =  $customer->order_details->sum('total_cost');
        $totalSold =  $customer->order_details->sum('total')  - $totalDiscount;


        $totalDiscountPerMonth = $customer->orders()->where('is_quotation', '0')->select(
        ...
        $totalPerMonth = $customer->orders()->where('is_quotation', '0')->select(
            DB::raw('DATE_FORMAT("%m %Y", created_at) as date'),
        ...
        $totalOrdersPerMonth = $customer->orders()->where('is_quotation', '0')->select(
            DB::raw('DATE_FORMAT("%m %Y", created_at) as date'),
    ...
    public function index1(Customer $customer)
    {
