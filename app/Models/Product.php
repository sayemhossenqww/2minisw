...
class Product extends Model
{
    ...
    public function getTableSalesViewPriceAttribute(): string
    {
        ...
        if ($hasExchangeRate) {
            $exchangeRate = config('settings')->exchangeRate;
            $price = currency_format($this->retailsale_price  * $exchangeRate * $this->in_stock, $hasExchangeRate);
            // return currency_format($this->sale_price * $this->in_stock, $hasExchangeRate) . " ({$price})";
            return currency_format($this->retailsale_price * $this->in_stock) . " ({$price})";
        }
    ...
    public function getWholeCostAttribute(): string
    {
        ...
        if ($hasExchangeRate) {
            $exchangeRate = config('settings')->exchangeRate;
            $price = currency_format($this->cost * $this->in_stock * $exchangeRate, $hasExchangeRate);
