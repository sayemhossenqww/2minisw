    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            let dataTable = $('#orders-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('api.quotations.index') }}",
                    dataSrc: 'data'
                },
                columns: [{
                        data: "number",
                        ...
                        render: function(data, type, row) {
                            var showUrl = "{{ route('quotations.show', ':id') }}";
                    ...
                    },
                    {
                        ...
                        data: function(data, type, dataToSet) {
                            ...
                            var printUrl = "{{ route('quotations.print', ':id') }}";
                            var convertUrl = "{{ route('quotations.convert', ':id') }}";
                            showUrl = showUrl.replace(':id', data.id);
                            editUrl = editUrl.replace(':id', data.id);
                            deleteUrl = deleteUrl.replace(':id', data.id);
                            printUrl = printUrl.replace(':id', data.id);
                            convertUrl = convertUrl.replace(':id', data.id);
                            return `<div class="dropdown d-flex">
                                ...
                                <x-dropdown-menu class="dropdown-menu-end" aria-labelledby="dropdownMenuButton1">
                                    ...
                                    <x-dropdown-item href="${printUrl}" target="_blank">
                                        <x-heroicon-o-printer class="hero-icon-sm me-2 text-gray-400" /> @lang('Print')
                                    </x-dropdown-item>

                                    <x-dropdown-item href="${convertUrl}">
                                        <x-heroicon-o-printer class="hero-icon-sm me-2 text-gray-400" /> @lang('Convert to Invoice')
                                    </x-dropdown-item>
